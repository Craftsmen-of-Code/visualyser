...
import { getCurrentDirPath } from "../getCurrentDirPath";
...
export class Parser {
  ...
  public updateTree(filePath: string): Tree | undefined {
    ...
    const getChildNodes = (node: Tree): void => {
      const { depth, filePath } = node;
      children.push({ depth, filePath });
    };
  ...
  #traverseTree(callback: Function, node = this.tree): void {
    if (!node) {
      return;
    }
  ...
  private getFileName(componentTree: Tree): string | undefined {
    ...
    if (!ext) {
      ...
      fileName = fileArray.find((fileStr) => fileStr.match(regEx)) as string;
  ...
  private getImports(body: { [key: string]: any }[]): ImportObject {
  ...
  private findVarDecImports(ast: { [key: string]: any }): string | boolean {
    ...
    for (let key in ast) {
      if (ast.hasOwnProperty(key) && typeof ast[key] === "object" && ast[key]) {
        const importPath: string | boolean = this.findVarDecImports(ast[key]);
        if (importPath) {
          return importPath;
        }
      }
  ...
  private checkForRedux(astTokens: any[], importsObj: ImportObject): boolean {
